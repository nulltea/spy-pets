...
async fn app(opts: Options) -> eyre::Result<()> {
    if let Some(command) = opts.command {
        match command {
            ...
            Command::Transfer(args) => transfer(args).instrument(info_span!("transfer")).await,
            Command::Uniswap(args) => {
                uniswap_swap(args)
                    .instrument(info_span!("uniswap::swap"))
                    .await
            }
            Command::BuyNft(args) => buy_nft(args).instrument(info_span!("opensea::buy")).await,
        }
...
async fn transfer(args: TransferArgs) -> anyhow::Result<()> {
    ...
    let lock_msg2 = alice
        .lock(
            bob_lock_msg,
            CovertTransaction::Swap(args.amount, target_address),
            args.withdraw_delay.map(|d| d.into()),
        )
...
where
    Fut: Future<Output = Vec<TypedTransaction>>,
{
    let name = args
        .wallet_name
        .unwrap_or_else(|| Text::new("Wallet name:").prompt().unwrap());
...
async fn uniswap_swap(args: UniswapArgs) -> anyhow::Result<()> {
    ...
    covert_contract_call(
        args.base_args.clone(),
        async move |address_from, address_to| {
            ...
            let call = dex
                .swap(amount, 0.5, &path, Some(address_to), None)
                .await
                .unwrap();
    ...
    loop {
        ...
        info!(
            "balance of {} is {} {}",
            args.base_args.target_address, tokens, args.target_erc20
        );
        sleep(Duration::from_secs(1));
        if tokens != 0.0 {
            break;
...
async fn buy_nft(args: BuyNFTArgs) -> anyhow::Result<()> {
    ...
    let block = chain
        ...
        .await?
        .unwrap();
    let _timestamp = block.timestamp.as_u64();
    ...
    let nfts = resp.body_json::<NftResponse>().await.unwrap();

    let nft = {
        let nft_contract = hex::encode_upper(args.nft_contract.to_fixed_bytes());
        let token_id = hex::encode_upper(args.token_id.encode());
        match nfts.response.into_iter().find(|e| {
            e.nftContract.trim_start_matches("0x").to_uppercase() == nft_contract
                && e.tokenId.trim_start_matches("0x").to_uppercase() == token_id
        }) {
    ...
    covert_contract_call(
        args.base_args.clone(),
        async move |address_from, _address_to| {
            let shared_wallet = KeylessWallet::new(address_from, chain.chain_id());
            ...
            let prepared_tx = resp.body_json::<TxResponse>().await.unwrap().response.tx;
    ...
    let target_address = Address::from_str(&args.base_args.target_address)
        .map_err(|e| anyhow!("error parsing target address: {e}"))?;
    if is_erc1155 {
        let balance = IErc1155::new(args.nft_contract, provider.clone())
            .balance_of(target_address, args.token_id)
            .call()
            .await?;
        info!(
            "{:?} owns {:?} ERC1155 NFTs with token id {:?}",
            target_address, balance, args.token_id
        );
    } else {
        let owner = IErc721::new(args.nft_contract, provider.clone())
            .owner_of(args.token_id)
            .call()
            .await?;
        info!(
            "Owner of ERC721 NFTs with token id {:?}: {:?}",
            args.token_id, owner
        );
    }
