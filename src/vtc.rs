...
use htlp::{lhp, BigUint, ToBigUint};
...
impl TimeLock for HTLP {
    fn lock(&self, w: &Scalar<Secp256k1>, _d: &Duration) -> VTC {
        let pp = lhp::setup::setup(self.security_bits, self.time_param.to_biguint().unwrap());
        let c = lhp::generate::gen(&pp, BigUint::from_bytes_be(&*w.to_bigint().to_bytes()));
...
impl TLock {
    pub async fn new<S: AsRef<str> + Copy>(
        host: S,
...
impl VariableVTC {
    ...
    pub async fn new_tlock<S: AsRef<str> + Copy>(
        host: S,
        chain_hash: impl AsRef<str>,
    ) -> anyhow::Result<Self> {
        Ok(Self {
...
impl VTC {
    ...
    pub fn unlock(self) -> impl Future<Output = Result<Scalar<Secp256k1>, JoinError>> {
        match self {
            VTC::HTLP(pp, c) => tokio::task::spawn_blocking(move || {
                Scalar::from_bigint(&BigInt::from_bytes(
                    &lhp::solve::solve(&pp, &c).to_bytes_be(),
                ))
            }),
            VTC::TLock(pp, c) => tokio::task::spawn(async move {
                ...
                let beacon = network
                    .get(pp.round_number)
                    .await
                    .expect("drand beacon is expected");
